...
class Bird {
  ...
  constructor(ctx){
    ...
    this.brain = new NeuralNetwork(3, 5, 1)//kuşa beyin veriyoruz
  ...
  update = () => {
    ...
    this.gravity = Math.min(4, this.gravity); // 5 max deger

    this.y += this.gravity;
  }


  think = () => {//kuşa beyin vermiştik, burada kuşa tahminde bulunduracağız
    //inputs
    //[bird.x,bird.y]
    //[clossestPipe.x, pipe.y]
    //[clossestPipe.x, pipe.y + pipe.height]
    ...
    //range(0-1)
    const result = this.bird.brain.predict(inputs)
    if(result < 0.5){
      this.jump();
